...
document.addEventListener('DOMContentLoaded', function() {
    // Bouton "Modifier mon choix" dans le récap (étape 2)
    const modifyChoiceBtn = document.querySelector('#selected-plan-summary button');
    if (modifyChoiceBtn) {
        modifyChoiceBtn.addEventListener('click', () => {
            navigateToStep(1); // Retour à l'étape 1 (choix du mode)
        });
    }
    ...
    // Correction: Toujours ré-attacher le handler du bouton "Modifier mon choix" après affichage de l'étape 2
    if (stepNumber === 2) {
        setTimeout(() => {
            const modifyChoiceBtn = document.querySelector('#selected-plan-summary button');
            if (modifyChoiceBtn) {
                modifyChoiceBtn.onclick = () => navigateToStep(1);
            }
        }, 0);
    }
    ...
    document.querySelector('.lw-btn-reserve.turbo-btn').addEventListener('click', () => selectMode('turbo'));
    document.querySelector('.lw-btn-reserve.express-btn').addEventListener('click', () => selectMode('express'));
    document.querySelector('.lw-btn-reserve.standard-btn').addEventListener('click', () => selectMode('standard'));
